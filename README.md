...
| Named + inline lists | `{list:countries}`, `{list:instruments}`, `{list:red,green,blue}` | 34 shipped categories — countries, capitals, US states, elements, constellations, presidents, dog breeds, Greek/Norse gods, Bible books, Shakespeare plays, tarot, moons, gemstones, musical instruments, car makers, sports, currencies… — or your own written in the query and shared in the URL. The large ones are generated from Wikidata (CC0) by `scripts/build-lists.mjs` and committed, so no network is needed at build or run time |
